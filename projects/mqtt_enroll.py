...
class MQTTEnrollManager:
    ...
    def _save_config(self):
        try:
            ...
            print(f"[Enroll ERROR] Failed to save config file {self.config_file_path}: {e}")
    ...
    def initialize_connection(self):
        if self.token and self.username:
            print("[Enroll INFO] Found existing token in config. Attempting direct connection...")
            return self.connect_with_token()
        else:
            print("[Enroll INFO] No token found in config. Attempting HTTP retrieval...")
            if self.retrieve_token_via_http():
                return self.connect_with_token()
            else:
                print("[Enroll ERROR] Failed to retrieve token via HTTP. Cannot connect.")
                if self.on_connection_status_change:
                    self.on_connection_status_change(False)
                return False
    def retrieve_token_via_http(self) -> bool:
        ...
        if domain:
            ...
            if not (domain.startswith("http://") and ':' in domain.split('//')[1]) and \
               not (domain.startswith("https://") and ':' in domain.split('//')[1]):
                 ...
                 # Nếu port là 80/443 thì không cần thêm
        else: # Không có domain, dùng broker
            scheme = "http" 
        ...
        try:
            # Không cần thay đổi verify ở đây vì đang dùng HTTP
            resp = requests.post(url, json=payload, timeout=10) # Không cần verify= cho http
            print(f"[Enroll DEBUG] HTTP Status Code: {resp.status_code}")
            resp.raise_for_status()
            data = resp.json()
        # ... Xử lý lỗi giữ nguyên ...
        except requests.exceptions.RequestException as e: # Bắt lỗi chung RequestException trước
            print(f"[Enroll ERROR] HTTP token request failed to {url}: {e}")
            return False
        except requests.exceptions.SSLError as ssl_err: # Sẽ không xảy ra nếu dùng http
             print(f"[Enroll ERROR] SSL Error connecting to {url}. Error: {ssl_err}")
             return False
            ...
            return False
        # ... Phần xử lý data JSON và lưu config giữ nguyên ...
            ...
            if topic == MQTT_REGISTER_RESPONSE_TOPIC:
                try:
                    ...
                    if token:
                        ...
                        if self.debug: print(f"[Enroll DEBUG] Registration via MQTT successful, received new token: {token[:10]}...")
                        self._save_config()
                    else:
    ...
    def connect_with_token(self):
        ...
        try:
            ...
            if broker_port == 8883:
                ...
                if self.mqtt_config.get("tls_insecure", False):
                     print("[Enroll WARN] TLS insecure mode enabled. Certificate verification disabled.")
    ...
    def on_connect_token(self, client, userdata, flags, rc, properties):
        ...
        if rc == 0:
            ...
            try:
                 ...
                 print(f"[Enroll INFO] Subscribed to {MQTT_REGISTER_RESPONSE_TOPIC}")
            except Exception as e:
                 print(f"[Enroll WARN] Failed to subscribe after connect: {e}")
            ...
            if rc == 5:
                ...
                self._save_config()
                print("[Enroll INFO] Attempting to retrieve a new token via HTTP...")
                time.sleep(5)
         ...
         if self.connected and self._client:
             try:
                 if self._client and self._client.is_connected():
                    ...
                    if result_info.rc == mqtt.MQTT_ERR_SUCCESS:
                    ...
                    elif result_info.rc == mqtt.MQTT_ERR_QUEUE_SIZE:
                        print(f"[Enroll WARN] MQTT publish failed (rc={result_info.rc}, queue full?). Queuing message.")
                     ...
                     print(f"[Enroll WARN] Client reported not connected during publish attempt. Queuing message.")
                     enqueue_outgoing_message(topic, payload_str, qos, user_properties)
    ...
    def flush_outbox(self):
        ...
        for entry_id, topic, payload, qos, props_json in items:
            if not self.connected or not self._client or not self._client.is_connected():
                 print("[Enroll WARN] MQTT disconnected during outbox flush. Stopping.")
                 break
            ...
            try:
                ...
                if result_info.rc == mqtt.MQTT_ERR_SUCCESS:
                    ...
                    print(f"[Enroll WARN] Failed to publish queued message id {entry_id} (rc={result_info.rc}, queue full?). Stopping flush.")
                    break
                else:
